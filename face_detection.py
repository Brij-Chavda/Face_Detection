...
def train_viola(t):
    with open("training_gauss_new.pkl", 'rb') as f:
        training = pickle.load(f)
    clf = ViolaJones(T=t)
    clf.train(training, 3000, 3000)
    evaluate(clf, training)
    clf.save(str(t))

def test_viola(filename):
    with open("test_gauss_new.pkl", 'rb') as f:
        test = pickle.load(f)
...
def train_cascade(layers, filename="Cascade"):
    with open("training_new.pkl", 'rb') as f:
        training = pickle.load(f)
...
#layers = [1,5,10,50]
#train_cascade(layers, 'fmdb')
#test_cascade('fmdb')
#for i in range(150, 260, 50):
train_viola(150)
test_viola(str(150))
